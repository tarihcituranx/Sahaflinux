...
def im_vakit_normalize(v: dict) -> dict:
    """İmsakiyem verisini standart formata çevir"""
    times = v.get("times", {})
    tarih_iso = v.get("date", "")          # "2026-03-10"
    ...
    hicri_str = hicri.get("full_date", "")
    if not hicri_str:
        hicri_str = f"{hicri.get('day','?')} {hicri.get('month_name','?')} {hicri.get('year','?')}"

    return {
        "imsak":      times.get("imsak", "—"),
        "gunes":      times.get("gunes", "—"),
        "ogle":       times.get("ogle",  "—"),
        "ikindi":     times.get("ikindi","—"),
        "aksam":      times.get("aksam", "—"),
        "yatsi":      times.get("yatsi", "—"),
        "tarih_kisa": tarih_kisa,
        "tarih_iso":  tarih_iso,
        "hicri_str":  hicri_str,
        "gun_adi":    gun_adi,
        # Ek bilgiler imsakiyem'de yok, boş bırak
        "kiblesaati":  "—",
        "gunesdogus":  "—",
        "gunesbatis":  "—",
        "gmt":         "+3",
...
with st.sidebar:
    ...
    if konumu_degistir or not st.session_state.konum_adi:
        ...
        if st.session_state.kaynak == "imsakiyem":
            # İmsakiyem: ülke → eyalet → ilçe (temiz Türkçe isimler)
            ...
            if ulkeler_im:
                ...
                s_ulke = st.selectbox("Ülke", ulke_display, index=varsayilan_ulke, label_visibility="collapsed", key="im_ulke")
                ...
                if secili_ulke_im:
                    ...
                    if eyaletler:
                        ...
                        s_eyalet = st.selectbox("Eyalet", eyalet_display, index=v_eyalet, label_visibility="collapsed", key="im_eyalet")
                        ...
                        if secili_eyalet_im:
                            ...
                            if ilceler_im:
                                ...
                                s_ilce = st.selectbox("İlçe", ilce_display, index=0, label_visibility="collapsed", key="im_ilce")
            ...
            # Emushaf: ülke → şehir → ilçe (bozuk isimler → adi_duzenle ile düzelt)
            ...
            if ulkeler_em:
                ...
                st.markdown('<p style="font-size:0.8em;color:#5a8aaa;margin-bottom:2px;">🌍 Ülke</p>', unsafe_allow_html=True)
                s_ulke = st.selectbox("Ülke", ulke_display, index=v_ulke, label_visibility="collapsed", key="em_ulke")
                ...
                if sehirler_em:
                    ...
                    s_sehir = st.selectbox("Şehir", sehir_display, index=v_sehir, label_visibility="collapsed", key="em_sehir")
                    ...
                    if ilceler_em:
                        ...
                        st.markdown('<p style="font-size:0.8em;color:#5a8aaa;margin:8px 0 2px;">📍 İlçe</p>', unsafe_allow_html=True)
                        s_ilce = st.selectbox("İlçe", ilce_display, index=0, label_visibility="collapsed", key="em_ilce")
