...
RATE_LIMIT_SANIYE = 16          # 4 istek/dakika → 60/4 = 15sn + 1 tampon
...
def groq_cevir(metin: str, baslik: str = "") -> str:
    """
    İngilizce açıklamayı Türkçeye çevirir.
    Rate limit: 4 istek/dakika → RATE_LIMIT_SANIYE bekleme.
    """
...
def ilan_karti_goster(ilan: dict, idx: int):
    ...
    with col1:
        if link:
            st.link_button("🎮 Hemen Al", url=link, use_container_width=True)
    ...
    with col3:
        ...
        if st.button(fav_ikon, key=f"fav_{idx}", use_container_width=True):
            if ilan_id in favs:
...
def main():
    ...
    col2.metric(
        "💰 Toplam Değer",
        f"${sum(float(d.get('worth','0') or 0) for d in ilanlar):.0f}",
    )
    ...
    # ── Tüm açıklamaları çevir butonu ──
    cevrilmemis = [
        d for i, d in enumerate(ilanlar[:10])
        if f"aciklama_{i}" not in st.session_state
    ]
    if cevrilmemis:
        st.caption(
            f"💡 '{len(ilanlar)}' ilandan ilk 10 tanesi için 'Türkçe Açıkla' butonunu kullanabilirsin. "
            f"Groq rate limit: 4 istek/dakika."
        )
